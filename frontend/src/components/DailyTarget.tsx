...
export function DailyTarget({
  ...
  const pctEarned = targetAmount > 0
    ? Math.min(Math.round((earnedToday / targetAmount) * 100), 100)
    : 0
  ...
  return (
    <div className={`rounded-2xl border-2 p-4 space-y-3 ${
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-gray-400">minimum penghasilan hari ini</p>
      </div>
        ...
        <div className="h-3 w-full rounded-full bg-gray-200 overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-500 ${
              isOnTrack ? 'bg-emerald-500' : pctEarned > 50 ? 'bg-yellow-500' : 'bg-red-500'
            }`}
            style={{ width: `${pctEarned}%` }}
          />
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-gray-400">{pctEarned}% tercapai</span>
      ...
      <div className="space-y-1 border-t border-gray-200 pt-2">
        <p className="text-xs font-semibold text-gray-400">Rincian target:</p>
        <div className="flex justify-between text-xs text-gray-500">
          <span>\uD83D\uDCB8 Pengeluaran harian</span>
          <span>{formatRupiah(breakdown.dailyExpense)}</span>
        </div>
        {breakdown.proratedRt > 0 && (
          <div className="flex justify-between text-xs text-gray-500">
            <span>\uD83C\uDFE0 Bulanan \u00F7 {breakdown.daysInMonth}</span>
            <span>{formatRupiah(breakdown.proratedRt)}</span>
          </div>
        )}
        <div className="flex justify-between text-xs text-gray-500">
          <span>\uD83D\uDCB3 Hutang \u00F7 {daysRemaining} hari</span>
